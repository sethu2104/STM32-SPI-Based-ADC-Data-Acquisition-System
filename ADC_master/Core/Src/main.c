...
/* USER CODE BEGIN PV */
uint16_t adc_value;
uint8_t tx_buffer[2]; // 2 bytes because adc_value is 16 bits
/* USER CODE END PV */
...
while (1)
  {
    ...
    // Prepare tx buffer (split 16-bit into 2 bytes)
    tx_buffer[1] = adc_value >> 8;      // High byte
    tx_buffer[0] = adc_value & 0xFF;    // Low byte
